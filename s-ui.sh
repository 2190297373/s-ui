...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        LOGI -e "${1} 正在运行，无需再次启动，如需重启请选择重启"
    else
        systemctl start $1
        sleep 2
        check_status $1
        if [[ $? == 0 ]]; then
            ...
            LOGE "启动 ${1} 失败，可能是因为启动时间超过两秒，请稍后检查日志信息"
        fi
    fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        if [[ $? == 1 ]]; then
            ...
            LOGE "停止 ${1} 失败，可能是因为停止时间超过两秒，请稍后检查日志信息"
        fi
    fi
...
restart() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "重启 ${1} 失败，可能是因为启动时间超过两秒，请稍后检查日志信息"
    fi
    if [[ $# == 1 ]]; then
        before_show_menu
    fi
...
show_enable_status() {
    check_enabled $1
    if [[ $? == 0 ]]; then
        echo -e "开机自启 ${1}: ${green}是${plain}"
    else
        echo -e "开机自启 ${1}: ${red}否${plain}"
    fi
}
...
disable_bbr() {
    ...
    if [[ $(sysctl net.ipv4.tcp_congestion_control | awk '{print $3}') == "cubic" ]]; then
        ...
        echo -e "${red}替换 BBR 为 CUBIC 失败。请检查您的系统配置。${plain}"
    fi
}
...
ssl_cert_issue() {
    ...
    *)
    ...
    local WebPort=80
    read -p "请选择使用的端口，默认将使用 80 端口:" WebPort
